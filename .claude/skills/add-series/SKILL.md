...
**Note:** This skill adds the series for the **current year only** (2026). To backfill older years, use `/support-year`.

1. Choose a series ID (short, kebab-case, URL-friendly)
2. Use `WebSearch` to find the official 2026 calendar, entry list, standings, and broadcast info
...
6. Create calendar file `src/data/<series>-2026.ts` with full event + session data in UTC
...
10. Create entries file in `src/data/entries/<series>-2026.ts` + register in `index.ts`
11. Create results file in `src/data/results/<series>-2026.ts` + register in `index.ts`
12. Create standings file in `src/data/standings/<series>-2026.ts` + register in `index.ts`
